...
<div class="dropdown">
  ...
  <ul class="dropdown-menu dropdown-menu-left" role="menu" aria-labelledby="dropdownMenu1">
    @if(Session::get('usuario')->tipo == 'gestion_clinica' || Session::get("usuario")->tipo === TipoUsuario::ENFERMERA_P || Session::get("usuario")->tipo === TipoUsuario::MASTER || Session::get("usuario")->tipo === TipoUsuario::MASTERSS || Session::get('usuario')->tipo == 'matrona_neonatologia' || Session::get('usuario')->tipo === TipoUsuario::SUPERVISORA_DE_SERVICIO)
    ...
    @endif
    {{-- <li role="presentation"><a role="menuitem" tabindex="-1" class='cursor' onclick="darAltaTransito({{$idCaso}}, {{$idLista}}, '{{$ficha}}', '{{$nombreCompleto}}'">Egreso</a></li> --}}
    ...
    @if(Session::get('usuario')->tipo != 'medico_jefe_servicio')
      ...
      {{-- <li role="presentation"><a onclick='tipoTransito({{$idLista}})'>Tipo tránsito</a></li> --}}
      ...
      @if(Session::get("usuario")->tipo === TipoUsuario::MASTER || Session::get("usuario")->tipo === TipoUsuario::MASTERSS || Session::get("usuario")->tipo === TipoUsuario::USUARIO)
      <li role="presentation"><a role="menuitem" tabindex="-1" class='cursor' onclick='trasladoUnidadHosp({{$idCaso}}, {{$idLista}})'>Salida de urgencia</a></li>
      @endif
    @endif  
  </ul>
</div>

...
@if($ruta_actual != null)
    <div class="text-center">
        <span class="pre alert alert-warning"><b>Origen:</b>  {{ $ruta_actual->hospital_origen }} <b>-> Destino:</b>  {{ $ruta_actual->hospital_destino }}  </span>
        
    </div>
    <br>
    <br>
    <div class="text-center">
        <span class="pre alert alert-warning"> <b>Hora Salida:</b> {{ $ruta_actual->hora_salida }} <b>-> Hora Llegada:</b> {{ $ruta_actual->hora_llegada_API }}</span>
    </div>
   
   <br>
   <br>
@endif
<form action='../listaRutas_guardar' method='POST' >
<div class="container">
    <div class="row">
        <div class="col-md-11 "
                    @if(count($errors)>0)
                        ...
                        <h4>Ingrese la ruta que desea para la ambulancia</h4>
                            ...
                            <table class = "table bg-info" id = "tabla" >
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td>
                                            {!! Form::select('origen[]',$establecimientos,$ruta->hospital_origen,['class' => 'origen form-control', 'id' => 'origen']) !!}
                                        </td>
                                        <td>
                                            {{ csrf_field() }}
                                            <input required class = "cambio form-control" type="text" name = "salida[]" value= "{{ $ruta->hora_salida }}">
                                        </td>
                                        <td> 
                                            <input readonly="readonly" class = "mostrarHora form-control"  type="text" name = "llegada[]" value="{{ $ruta->hora_llegada_API }}">
                                        </td>    
                                        ...
                                        <td>
                                            {!! Form::select('destino[]',$establecimientos,$ruta->hospital_destino,['class' => 'form-control', 'id' => 'destino']) !!}
                                        </td>
                                        ...
                                        <td>
                                            <input class="idPaciente" type="hidden" name="idPaciente[]" value="{{$ruta->paciente_id}}">
                                        	@if( $ruta->paciente_id != null)
                                                @if($ruta->rut !=null)
                                                    <input readonly="readonly" name="paciente[]" class = "paciente form-control"  type="text" data-indice={{$i}} value="{{ $ruta->rut.'-'.$ruta->dv}}">
                                                @else
                                                    <input readonly="readonly" name="paciente[]" class = "paciente form-control"  type="text" data-indice={{$i}} value="{{ $ruta->nombre.' '.$ruta->apellido_paterno}}">
                                                @endif
                                            	
                                            @else
                                            	<input readonly="readonly" name="paciente[]" class = "paciente form-control"  type="text" data-indice={{$i}} value="No tiene">
                                            @endif
...
$('body').on('click','#listaPacientes', function(e){  
    ...
    function cargarPacientes(idPaciente,nombre,rut,indice_origen){

        $("#tabla tr").each(function (indice, elemento) {

            if($('.paciente').eq(indice).data('indice') == indice_origen){
                $('.idPaciente').eq(indice).val(idPaciente);

                console.log(indice_origen);
                if (rut == false) {
        ...
        $("#adicional").on('click', function(){
            // $("#tabla").append("<td><input name= 'hola[]'></td>");

 			...
 			$.ajax({
 				...
 				success: function(data){
 					...
 					if(data.exito){
						...
						didOpen: function() {
							setTimeout(function() {
						@if(Session::get("usuario")->tipo === TipoUsuario::IAAS)
						window.location.href="{{URL::to('/')}}/index/camas/<?php echo $esta;?>";
						@elseif(Session::get("usuario")->tipo === TipoUsuario::MASTER)
						window.history.back();
						@endif
							}, 2000)
						},
 				...
 				error: function(error){
					 swalError.fire({
						title: 'Error',
						text:"Error al actualizar infeccion"
						});
 					console.log(error);
 				}
 			});
 		...
 		$("#formLiberar").submit(function(evt){
 			...
 			$.ajax({
 				...
 				error: function(error){
					 swalError.fire({
						title: 'Error',
						text:"Error al cerra la infección"
						});
 					console.log(error);
 				}
 			});
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<div class="row">
					<div class="form-group col-md-12">
						<label for="horas" class="col-sm-2 control-label">Motivo de liberación: </label>
						<div class="col-sm-10">
